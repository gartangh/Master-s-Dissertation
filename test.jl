...
function chunk(t::Tensor{Float32,4}, chunks::Int64)
  ...
  ts = [t.ptr for _ = 1:chunks]

  Torch.atg_chunk(ts, t.ptr, chunks, dim)

  vtop::Vector{Tensor{Float32,4}} = [t for _ = 1:chunks]
  for i = 1:chunks
...
function (group::GroupedConvolutions)(input::Tensor{Float32,4})
  ...
  if group.split == true
    ...
    group.connection([
      path(chunks[path_index]) for (path_index, path) in enumerate(group.paths)
    ]...)
  else
...
function main()
  ...
  c = Conv((2, 2), 4 => 2)
  op = c(ip)
  g = GroupedConvolutions(
    (results...) -> cat(results..., dims = 3),
    Conv((3, 3), 2 => 2, pad = (1, 1), stride = (1, 1)),
    Conv((3, 3), 2 => 2, pad = (1, 1), stride = (1, 1)),
    split = true,
  )
  op = g(ip)
