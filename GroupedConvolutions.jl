...
# sum
println("SUM")
group = GroupedConvolutions(+, [create_path_sum(nmaps, npaths) for _ in 1:npaths]..., split=true)
println(group)
...
# concat
println("CAT")
group = GroupedConvolutions((results...) -> cat(results..., dims=3), [create_path_cat(nmaps, npaths) for _ = 1:npaths]..., split=true)
println(group)
